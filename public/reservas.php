...
.logo-externo { text-align: center; margin-top: 140px; margin-bottom: 20px; width: 100%; }
.logo-externo img { max-width: 220px; height: auto; filter: drop-shadow(0 0 10px rgba(255, 255, 255, 0.2)); }
...
@media (max-width: 600px) {
    .logo-externo { margin-top: 100px; }
    .logo-externo img { max-width: 160px; }
    .reserva-container { margin: 0 15px 50px 15px; padding: 30px 20px; }
    .eventos-reserva { grid-template-columns: 1fr; }
    .evento-card img { height: 280px; }
}
</style>
</head>
...
<div class="logo-externo">
    <img src="img/logo/logo Luna.png" alt="Logo La Luna">
</div>

<div class="eventos-reserva" id="eventosReserva">
    <p style="grid-column:1/-1; text-align:center; color:#888;">Cargando eventos disponibles...</p>
</div>
...
<div class="reserva-container" id="formContainer">
    ...
    <form id="formReserva">
        ...
        <button type="submit" class="btn-reserva">SOLICITAR RESERVA</button>
    </form>
    ...
    <div id="msgExito" style="display:none; text-align: center; margin-top: 20px;">
        <p style="color: #00ff00; font-weight: bold;">¡Reserva enviada con éxito!</p>
        <p style="color: #888; font-size: 0.9rem;">Nos pondremos en contacto contigo por teléfono para confirmar.</p>
        <a href="index.php" style="color: #fff; text-decoration: underline; font-size: 0.8rem;">Volver al inicio</a>
    </div>
</div>
...
async function cargarEventos() {
    try {
        ...
        const eventosFiltrados = events.filter(e => {
            ...
            const oculto = Number(e.oculto) === 1;
        ...
        contenedor.innerHTML = eventosFiltrados.map(e => {
            ...
            return `
                <div class="evento-card" onclick="seleccionarEvento('${e.date}', '${titulo.replace(/'/g, "\\'")}', '${e.image}')">
...
document.getElementById('formReserva').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch('api/crear_reserva.php', {
            method: 'POST',
            body: formData
        });
